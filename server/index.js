...
// Image Proxy Endpoint with Full Parameter Preservation
app.get('/api/proxy-image', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.log('Proxy image fetch HTTP failed:', response.status, imageUrl.slice(0, 80));
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', service: 'InstaPulse AI Backend', timestamp: new Date().toISOString() });
});

app.listen(PORT, () => {
  console.log(`🚀 InstaPulse AI Server running on http://localhost:${PORT}`);
});
